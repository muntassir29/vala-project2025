...
import { useEffect, useState } from "react";
import axiosInstance from "../api/axiosInstance"; // <-- axiosInstance importé
import { useAuth } from "../context/AuthContext";
import {
  BarChart,
  Bar,
...
const Stats = () => {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      setError("");
      try {
        const [resStrategies, resMonthly] = await Promise.all([
          axiosInstance.get("/trades/strategies/stats", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axiosInstance.get("/trades/stats/monthly", {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
        setStrategyStats(resStrategies.data);
        setMonthlyStats(resMonthly.data);
      } catch (err) {
        console.error("Erreur récupération statistiques:", err);
        setError("Erreur récupération des statistiques.");
      } finally {
        setLoading(false);
      }
    };
